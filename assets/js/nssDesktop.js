...
function sliderButtons() {
  ...
  buttonPrevious.addEventListener("click", function () {
    if (idButton === 0) buttons[buttons.length - 1].click();
    else buttons[idButton - 1].click();
  });
  buttonNext.addEventListener("click", function () {
    if (idButton === buttons.length - 1) buttons[0].click();
    else buttons[idButton + 1].click();
  });
}
...
const expanderSubCats = document.getElementsByClassName(
  "gf-expander__subcategory-list"
);
...
document.onclick = function () {
  if (
    document.getElementById("nssSuggestionBox") != null &&
    document.getElementById("nssSuggestionBox").hasAttribute("style")
  ) {
 ...
 * in css its set top:28% and thats same value used on production (ITS NOT ACTUALLY CENTERED THO)
...
const nextButtons = document.getElementsByClassName(
  "product-slider__control-next"
);
if (prevButtons != null) {
  for (let prevButton of prevButtons) {
    const swipePrev =
      prevButton.parentElement.parentElement.parentElement.lastElementChild
        .swiper;
...
if (nextButtons != null) {
  for (let nextButton of nextButtons) {
    const swipeNext =
      nextButton.parentElement.parentElement.parentElement.lastElementChild
        .swiper;
...
const descriptionButton = document.querySelector(
  ".gf-archive-description-button"
);
if (descriptionButton !== null) {
  descriptionButton.addEventListener("click", () => {
    document
      .querySelector(".gf-archive-description p")
      .classList.toggle("gf-display-category-description");
  });
}
...
const billingCompanySettings = document.querySelector(
  "p#billing_company_field > label > span"
);
const billingPibSettings = document.querySelector(
  "p#billing_pib_field > label > span"
);
...
if (billingPostcode !== null) {
  ...
  billingCompany.addEventListener("click", (e) => {
    ...
    if (billingCompany.checked) {
      document
        .getElementById("billing_company_field")
        .classList.add("validate-required");
      document
        .getElementById("billing_pib_field")
        .classList.add("validate-required");
    ...
    if (this.status >= 200 && this.status < 400) {
      if (this.responseText.trim() !== "") {
        document.getElementById(
          "nssSuggestionBox"
        ).innerHTML = this.responseText.trim();
...
function viewCountAjax() {
  let request = new XMLHttpRequest();
  let productNum = document
    .querySelector("[id^=product]")
    .getAttribute("id")
    .split("-")[1];
...
//update post code when city is selected
